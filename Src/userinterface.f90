...
!         read(11,'(7f10.8,A)') box_dimensions(i,1:6),box_buff, block_ucells(i)
...
&       def_block(n_def_groups),  def_type(n_def_groups),def_treshold(n_def_groups))
	   do i=1,n_def_groups
...
101 continue
	...
	!print*,'Number of total chains are : ', n_chains,n_cont_chains,n_amorph_chains,nfunctional
	write(*,'(A)') '# of Amorphous chains |# of Continuous Chains |# of Functional Chains | Total # of Chains '
...
&    iostat=pre_analize()
    ...
    if ( n_cont_chains .ne. 0 ) then
      ...
      !write(*,'(A)') 'No continuous chains needed'
    end if
	!------------------------------------------
    ! Create Amorphous Chains
	!------------------------------------------
...
!	 if( p .eq. 0 ) &
!&	 write(*,'(A6,i3,x,A6,i3,A6,x,3f6.2,x,A6,i4)') &
!&	 'Chain: ',i,' Bead :',j,' Pos : ',vec,' Inside : ',p
	 end do
	 end do
...
&                    two_chain_interaction(chains(i),chains(j))
	  ...
	  do i=n_cont_chains+1,n_chains
		...
		!print*,i,total_penalty
	  end do
	  !pause '---'
	  total_penalty=self_penalty+inter_penalty
	 
   
!       print*,q, self_penalty,inter_penalty,total_penalty
...
!		write(*,'(4f12.4)'),total_penalty,save_penalty,exp((-total_penalty+save_penalty)/mcTemp),rn
...
!	  print*, 'Blocks allocated : ', allocated(blocks),size(blocks) 
